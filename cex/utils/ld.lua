...
read_libdata = function(file)
  ...
  assert(handle:read(4) == "onyC", "invalid CEX signature for " .. file)
  ...
  if not lib then
    io.stderr:write("ld.cex: " .. file .. " not marked as library\n")
    os.exit(1)
  end
  local data = ""
  if not static then
    while nlinks > 0 do
      ...
      for i=1, #lib_search, 1 do
        ...
        if handle then
          handle:close()
          io.stderr:write("ld.cex: linking to library ", name, "\n")
          data = data .. read_libdata(path)
          break
        end
        if i == #lib_search then
          io.stderr:write("ld.cex: could not find library " .. name .. "\n")
          os.exit(1)
        end
      end
    end
...
for i, arg in ipairs(args) do
  ...
  elseif arg:sub(1,1) == "-" then
    ...
    elseif arg == "i" or arg == "include" then
      lib_search[#lib_search + 1] = args[i+1] .. "/?.cex"
      skipnext = true
    else
...
for i, file in ipairs(to_link) do
  ...
  for i=1, #lib_search, 1 do
    ...
    if i == #lib_search and not handle then
      io.stderr:write("ld.cex: could not find library " .. file .. "\n")
      os.exit(1)
    else
      handle:close()
      io.stderr:write("ld.cex: linking to library ", file, "\n")
      path = _path
    end
  end
  if static then
